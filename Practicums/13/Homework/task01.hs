...
roundTwoDigButWithMagic :: (RealFrac a) => a -> a -- week 9 task 01
roundTwoDigButWithMagic = (/100) . fromIntegral . round . (* 100)

average :: (RealFrac a) => BTree a -> a
average tree = roundTwoDigButWithMagic (sumTree tree / (fromIntegral (size tree)))
...
areEqual first second = traverseDFS first == traverseDFS second
...
setLevels tree = helper 0 tree 
    where 
        ...
        helper _ Nil = Nil
        helper curretLevel (Node value left right) = Node (curretLevel, value) (helper (curretLevel + 1) left) (helper (curretLevel + 1) right)
